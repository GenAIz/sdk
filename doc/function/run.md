...
The command the proceeds to run the image built under a **disposable** container using the mount points specified under the `Genaiz.yaml` file or from the options on the command line.

### context

Context for the run command establishes the working directory for all the specified mount paths and for the [build](build.md) tasks as well.
...
* if mount-in resolve to an empty value, no [SF_INPUT_PATH](index.md#sf_input_path) host mount point will be provided to the function's container.
...
* if mount-out is not specified, the default key `sf.run.log` will be read from the Smart Function `Genaiz.yaml`
...
* if an environment variable specified does not validate with its associated property specification's type, the command will return an error: ``
...
The prefix field is not particularly useful to run, since it disposes of the created containers when they exit. If you do look for attaching on long-running containers however the name of the container will be displayed under `docker container ls` and the [list](list.md) command.
