...
```
genaiz sf start --context|-c=PATH --file|-f=FILE --tag=LOCAL --version|-v=VERSION \
...
* if mount-in is not specified, the default key `sf.start.input` will be read from the Smart Function `Genaiz.yaml`
* if `sf.start.input` is not specified, the value under `sf.run.input` will be read from the Smart Function `Genaiz.yaml`
...
* if mount-out is not specified, the default key `sf.start.output` will be read from the Smart Function `Genaiz.yaml`
* if `sf.start.output` is not specified, the value under `sf.run.output` will be read from the Smart Function `Genaiz.yaml`
...
* if mount-log is not specified, the default key `sf.start.log` will be read from the Smart Function `Genaiz.yaml`
...
* if mount-var is not specified, the default key `sf.start.var` will be read from the Smart Function `Genaiz.yaml`
